...
async function initCart() {
    ...
    cart = JSON.stringify(results.cart);
    cookie.set("cart", window.btoa(cart));
}
...
export async function addProduct(idProduct, idProductAttribute = 0) {
    console.log(`ADD PRODUCT ${idProduct}`);
    if(cart['products'][idProduct] != undefined)
        cart['products'][idProduct]++;
    else
        cart['products'][idProduct] = 1;
